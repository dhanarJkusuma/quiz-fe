...
const Quiz = () => {
    ...
    const getUserName = (initiateData) => {
        let sessionID = localStorage.getItem("session-user-id");
        if(initiateData.player_1_id == sessionID){
            return initiateData.player_1;
        }
        if(initiateData.player_2_id == sessionID){
    ...
    const answerQuestion = (opt, token) => {
        if(state.showAnswer){
          return
        }
        ...
        state.socket.emit('answer', JSON.stringify(payload));
        setState(prevState => ({
          ...prevState,
          showAnswer: true,
          options: options,
    ...
    return (
        <div>
            <br/>
            { state.inquestion && state.displayNotification && <Notification correct={ state.correctAnswer } /> }
            <br/>
            {
            state.inquestion && 
            <div>
                ...
                <ListGroup>
                {
                    ...
                    return (
                        ...
                        key={index} 
                        action 
                        onClick={() => answerQuestion(item, state.tokenAnswer)}>
                        { item.answer }
                        </ListGroupItem>
                    )
                    })
            ...
            state.idle && 
            <div>
                <Button color="primary" onClick={ connectLobby } disabled={ state.inlobby }>Connect Lobby</Button> 
                &nbsp;
                <Button color="success" onClick={() => { initQuiz(state.socket) }} disabled={ !state.inlobby }>Start Quiz</Button>
            </div>
            }
            <br />
